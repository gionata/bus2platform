...
bool MathModelBP::setLinkConstraints()
{
	...
	for (unsigned int k = 0; k < _graphs.numPlatforms(); k++) {
		...
		set_mat(_lp, row_no, _uk_start + k, -dwells_counter);
		set_constr_type(_lp, row_no, LE);
...
/*
unsigned char MathModelBP::writeModelLP_solve(char *filename)
{
	return write_lp(_lp, filename);
}

unsigned char MathModelBP::writeModelCPLEX(char *filename)
{
